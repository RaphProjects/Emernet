...
NORMALIZATION_FILE = "arena_normalization.json"
NORMALIZATION_OVERRIDES = {}
DEFAULT_SPEED_BALANCE = {
    "Unified": 0.3,
    "Rich": 0.3,
    "All": 0.3,
}
SPEED_BALANCE_FILE = "arena_speed_balance.json"
SPEED_BALANCE_OVERRIDES = {}


def _load_normalization_overrides():
    if not os.path.exists(NORMALIZATION_FILE):
        return {}
    try:
        with open(NORMALIZATION_FILE, "r", encoding="utf-8") as f:
            loaded = json.load(f)
        if "default" in loaded and "Rich" not in loaded:
            loaded["Rich"] = loaded["default"]
        return {k: v for k, v in loaded.items() if k in DEFAULT_NORMALIZATION}
    except Exception:
        return {}


def _save_normalization_overrides():
    with open(NORMALIZATION_FILE, "w", encoding="utf-8") as f:
        json.dump(NORMALIZATION_OVERRIDES, f, indent=2)


def get_normalization_values():
    return {
        key: {**values, **NORMALIZATION_OVERRIDES.get(key, {})}
        for key, values in DEFAULT_NORMALIZATION.items()
    }


def set_normalization_values(values):
    if "default" in values and "Rich" not in values:
        values["Rich"] = values["default"]
    for key in ("Unified", "Rich", "All"):
        if key not in values:
            continue
        current = {**DEFAULT_NORMALIZATION[key], **NORMALIZATION_OVERRIDES.get(key, {})}
        for field in ("avg_learn", "std_learn", "avg_speed", "std_speed", "avg_simp", "std_simp"):
            if field in values[key]:
                current[field] = float(values[key][field])
        if current["std_learn"] <= 0 or current["std_speed"] <= 0 or current["std_simp"] <= 0:
            raise ValueError("Normalization std values must be positive")
        NORMALIZATION_OVERRIDES[key] = current
    _save_normalization_overrides()
    return get_normalization_values()


NORMALIZATION_OVERRIDES.update(_load_normalization_overrides())


def normalization_key_for_modules(modules):
    if modules == "Unified":
        return "Unified"
    if modules == "All":
        return "All"
...
def _load_speed_balance_overrides():
    if not os.path.exists(SPEED_BALANCE_FILE):
        return {}
    try:
        with open(SPEED_BALANCE_FILE, "r", encoding="utf-8") as f:
            loaded = json.load(f)
        if "default" in loaded and "Rich" not in loaded:
            loaded["Rich"] = loaded["default"]
        return {
            ...
            if k in DEFAULT_SPEED_BALANCE and 0.0 <= float(v) <= 1.0
        }
    except Exception:
        return {}


def _save_speed_balance_overrides():
    with open(SPEED_BALANCE_FILE, "w", encoding="utf-8") as f:
        json.dump(SPEED_BALANCE_OVERRIDES, f, indent=2)


def get_speed_balance_values():
    return {
        key: float(SPEED_BALANCE_OVERRIDES.get(key, value))
        for key, value in DEFAULT_SPEED_BALANCE.items()
    }


def set_speed_balance_values(values):
    if "default" in values and "Rich" not in values:
        values["Rich"] = values["default"]
    for key in ("Unified", "Rich", "All"):
        if key not in values:
        ...
        SPEED_BALANCE_OVERRIDES[key] = value
    _save_speed_balance_overrides()
    return get_speed_balance_values()


SPEED_BALANCE_OVERRIDES.update(_load_speed_balance_overrides())
...
class Arena:
    def __init__(self, n_fights=1, architecture_size=16, arena_contestants=3, dataset_size = 256+64,
                  train_test_split= 0.7, generation_type="agnostic", verbose=True, report=False, pcp=0.38,
                    cpu=False, simp_bal=0.36, speed_bal=None, simp_opp_bal=0.2293, modules = "Unified"):
        ...
        self.cpu = cpu
        self.module_types = modules
        self.simp_bal = 0 # simplicity didn't show any advantage
        norm = get_normalization_values()[normalization_key_for_modules(modules)]
        self.avg_learn = norm["avg_learn"]
        self.std_learn = norm["std_learn"]
        self.avg_speed = norm["avg_speed"]
        self.std_speed = norm["std_speed"]
        self.avg_simp = norm.get("avg_simp", self.avg_learn)
        self.std_simp = norm.get("std_simp", self.std_learn)
        
        self.speed_bal = get_speed_balance_values()[normalization_key_for_modules(modules)] if speed_bal is None else speed_bal
    ...
    def get_scores(self, arch_1, arch_2, input = None, get_penalties=False, outerfunction="sqrt", randomizeHP=False,
                    pcp=None, uniform=False, input_noise = 0.05, output_noise = 0.01, get_delays=False, max_duration=None):
        ...
        if randomizeHP:
            ...
            learner_1.fit(train_input, train_target_1.detach(), verbose=self.verbose, lr=random_lr, max_iter=random_max_iter, batch_size=min(train_size,random_batch_size), patience = random_patience, min_delta = random_min_delta, cpu = self.cpu, max_duration=max_duration)
            l1_delay = time.time() - l1_start_time
            l2_start_time = time.time()
            learner_2.fit(train_input, train_target_2.detach(), verbose=self.verbose, lr=random_lr, max_iter=random_max_iter, batch_size=min(train_size,random_batch_size), patience = random_patience, min_delta = random_min_delta, cpu = self.cpu, max_duration=max_duration)
            l2_delay = time.time()-l2_start_time
        else:
            l1_start_time = time.time()
            learner_1.fit(train_input, train_target_1.detach(), verbose=self.verbose, lr=0.01, max_iter=200, batch_size=min(train_size,max_batch_size), patience = 10, min_delta = 1e-7, cpu = self.cpu, max_duration=max_duration)
            l1_delay = time.time() - l1_start_time
            l2_start_time = time.time()
            learner_2.fit(train_input, train_target_2.detach(), verbose=self.verbose, lr=0.01, max_iter=200, batch_size=min(train_size,max_batch_size), patience = 10, min_delta = 1e-7, cpu = self.cpu, max_duration=max_duration)
            l2_delay = time.time()-l2_start_time
    ...
    def occam_selection(self, n_archs=16, verbose=False, randomizeHP=True, use_delays=True,
        ...
        generator = Generator(generation_type=self.generation_type, module_types=self.module_types)
        ...
        norm_simp = [(s - self.avg_simp) / self.std_simp for s in simplicity_scores]
    ...
    def occam_test(self, ori_architectures, n_archs=8, verbose=False, randomizeHP=True,
        ...
        norm_simp = [(s - self.avg_simp) / self.std_simp for s in simplicities]
    ...
    def simp_bal_opp_selection(self, n_archs=16, verbose=False, randomizeHP=True, simp_bal=None, simp_opp_bal=None):
        ...
        simplicities = [(s - self.avg_simp) / self.std_simp for s in simplicities]
    ...
    def tune_simp_opp_bal(self, n_archs=16, verbose=True, randomizeHP=True, n_rounds=3):
        ...
        for r in range(n_rounds):
            ...
            simp_z = [(s - self.avg_simp)/self.std_simp for s in simp_raw]
    ...
    def test_real_correlation(self,architectures,n_archs_test=16,simp_bal=None, simp_opp_bal=None, real_iter = 150, verbose = True, save_path="correlation_data.csv", useSimpBalOpp = True):
        ...
        if useSimpBalOpp:
            ...
            for ntested_arch in range(len(architectures)):
                ...
                simp_tested_z = (simp_tested - self.avg_simp) / self.std_simp
                ...
                for j in range(n_opp):
                    ...
                    simp_opps.append((s - self.avg_simp) / self.std_simp)
            ...
            for i, arch in enumerate(architectures):
                ...
                norm_simp = [(s - self.avg_simp) / self.std_simp for s in raw_simp]
    ...
    def rl_search(self, n_phase_a_episodes=500, n_phase_b_episodes=500,
                   n_candidates_per_step=10, module_set="Unified",
                   retrain_frequency=50, verbose=True):
        ...
        result = searcher.run(
            n_phase_a_episodes=n_phase_a_episodes,
            n_phase_b_episodes=n_phase_b_episodes,
