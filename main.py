...
unifArch = Architecture()
unifArch.add_node(0,Input())
unifArch.add_node(1,LearnableParameter((1,4,8)))
unifArch.add_node(2,EinsteinAggregator())
unifArch.add_edge(0,2)
unifArch.add_edge(1,2)


mlp = arena.make_mlp([16,8])

print(arena.get_scores(unifArch,mlp,get_delays=True))
print(arena.get_scores(unifArch,mlp,get_delays=True))
#arena.get_learnability_and_delays_data(verbose=True)
'''
generator = Generator(generation_type="agnostic")
rnd_archs = [generator.generate(12) for _ in range(48)]
small_mlp = arena.make_mlp([8,16])
